...
class Scene {
    ...
    addEventListeners() {
        ...
        this.rootEl.addEventListener("mousemove", (event) => {
            ...
            if (!moveOffset) {
                moveOffset = Vector.sub(this.selectedHullCircle.circle.translation, this.mouseDownPosition)
            }

            // Normal mode - move circle
            this.selectedHullCircle.circle.translation = Vector.add(new Vector(clientX, clientY), moveOffset)
        ...
        this.rootEl.addEventListener('mouseup', () => {
            if (this.selectedHullCircle) {
                const circleEl = document.getElementById(this.selectedHullCircle.circle.id);
                if (circleEl) {
